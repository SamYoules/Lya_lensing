...
def create_small_blob_kappa(nside=512, phi0=np.pi/2, theta0=1.1):
    ''' Generate kappa map with a blob in the center '''
    npix = nside**2*12
    kappa = np.zeros(npix)
    theta, phi = hp.pix2ang(nside,np.arange(npix))
    #phi[phi>np.pi] -= 2*np.pi
    kappa = SphericalMap(np.exp(-((phi-phi0)**2+(theta-theta0)**2)/(2*0.01**2)))
    kappa.compute_deriv()
    return kappa
...
def create_gaussian_kappa(ell, cell, nside=1024, seed=1):
    ...
    #obsk = hp.sphtfunc.synfast(cell, nside=nside, lmax=2*nside-1,
                               #pol=False)
    obsk = hp.sphtfunc.synfast(cell, nside=nside, lmax=3*nside-1,
                               pol=False)
